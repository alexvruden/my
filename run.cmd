...
if %srv_trigger% equ 1 (
	if %task% EQU 0 (
		rem echo.[%c2%G[36m[[0m задача в планировщике заданий: [32mесть[%c8%G[36m][0m
		wmic process Where Name="cmd.exe" Get CommandLine,ProcessId /Format |find "run_agent.cmd" 1>nul 2>&1
		if !errorlevel! EQU 0 (
			if exist %home%\bin\agent_status ( set /p foo=<%home%\bin\agent_status ) else ( set "foo=неизвестное состояние..." )
