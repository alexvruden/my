...
for /F "skip=1 eol=# tokens=1,2 delims==" %%a in (%home%\run.config) do (
	set foo=%%a
	set /a aoo=0
	for %%i in (term_trigger srv_trigger strategy_trigger param_trigger show_popup language new_version_available check_update eula daemon custom_strategy IPsetStatus ip_router agent_mode agent_start_strategy agent_start_params) do (
...
for /F "skip=1 eol=# tokens=1,2 delims==" %%a in (%home%\run.config) do (
	...
	if !aoo! equ 1 (
		set goo=%%b
		call set coo=%%%%~a%%
		rem if not "x!coo!"=="x!goo!" (
		REM echo.config !foo!=[!goo!]
		REM echo.current !foo!=[!coo!]
		REM goto:@F
		REM )
	...
	for /F "usebackq tokens=1* delims=:" %%a in (`"findstr /n ^^^^ !foo!"`) do (
		...
		if "x!fletter:~0,1!"=="x$" (
			...
			if !parse_desync! equ 1 (
				...
				if "x!foo:~0,2!"=="x0x" (
					set "profile_param=!profile_param! !fletter!=!sletter!"
				) else if "x!foo:~0,4!"=="xRND" (
					call:@rnd_tls_gen foo
...
set "home_find=%home%\strategy\find\%date_runing%"
if not exist %home_find% md %home_find% 1>nul
...
set /a coo=1
:@curl_cmd_https_loop
set curl_cmd_scan%coo%=--connect-to !DOMAINS%coo%!::[!ip_dom%coo%!]:%HTTPS_PORT% -ISs %_CURL_OPT% -A "%curl_user_agent%" --max-time %CURL_MAX_TIME% %TLSver% %TLSmax% https://!DOMAINS%coo%!
set curl_cmd_scan_long%coo%=--connect-to !DOMAINS%coo%!::[!ip_dom%coo%!]:%HTTPS_PORT% -Ss %_CURL_OPT% -A "%curl_user_agent%" --max-time 60 %TLSver% %TLSmax% https://!DOMAINS%coo%!
set /a coo+=1
...
set /a coo=1
:@curl_cmd_http3_loop
set curl_cmd_scan%coo%=--connect-to !DOMAINS%coo%!::[!ip_dom%coo%!]:%QUIC_PORT% -Ss %_CURL_OPT% -A "%curl_user_agent%" --max-time %CURL_MAX_TIME% --http3-only https://!DOMAINS%coo%!
set curl_cmd_scan_long%coo%=--connect-to !DOMAINS%coo%!::[!ip_dom%coo%!]:%QUIC_PORT% -Ss %_CURL_OPT% -A "%curl_user_agent%" --max-time 60 --http3-only https://!DOMAINS%coo%!
set /a coo+=1
...
set /a coo=1
:@curl_cmd_http_loop
set curl_cmd_scan%coo%=--connect-to !DOMAINS%coo%!::[!ip_dom%coo%!]:%HTTP_PORT% -SsD %home_find%\blk-hdr.txt %_CURL_OPT% -A "%curl_user_agent%" --max-time %CURL_MAX_TIME% http://!DOMAINS%coo%!
set curl_cmd_scan_long%coo%=--connect-to !DOMAINS%coo%!::[!ip_dom%coo%!]:%HTTP_PORT% -SsD %home_find%\blk-hdr.txt %_CURL_OPT% -A "%curl_user_agent%" --max-time 60 http://!DOMAINS%coo%!
set /a coo+=1
...
goto:@F
:@F1
echo.# !curl_cmd_scan%coo%! >%home_find%\run-!DOMAINS%coo%!-%_PORT%-%_HTTP%.%find_strategy_position_end%
:@F
set /a coo+=1
...
echo.[%line_jump_num%H[%_pos%G[[33mi[m][%pos%G%lmess159% '[33m..\strategy\find\%date_runing%\run-[[36mDOMAINS[33m]-%_PORT%-%_HTTP%.*[m'
set /a line_jump_num+=1
...
set /a line_jump_num+=1
echo.[%line_jump_num%H[%_pos%G[[33mi[m][%pos%G%lmess162% '[33m..\strategy\find\%date_runing%\[[36mDOMAINS[33m]\%_HTTP%\*-[[36m%lmess163%[33m][m'
set /a line_jump_num+=1
...
%winwsdir%\winws.exe --daemon %wf_foo% --hostlist-domains=!DOMAINS%coo%! %winws_arg_str_% 1>nul 2>&1
if %errorlevel% neq 0 goto:@F3
set /a curl_ret_code_0=1000
set /a c_count=1
set curl_ret_code=0
:@repeats_loop
%CURL% !curl_cmd_scan%coo%! 1>nul 2>&1
...
<nul set /p =8[%pos6%G%curl_ret_code_str_s%[%pos7%G%c_count%[?25l[8m
if %c_count% neq %REPEATS% goto:@F
set foo=0
if %curl_ret_code% neq 0 goto:@F1
<nul set /p =8[%pos6%G%empty6%[%pos7%G%empty7%[?25l[8m
<nul set /p =8[%pos6%G%lmess174%[?25l[8m
%CURL% !curl_cmd_scan_long%coo%! 1>nul 2>&1
if %errorlevel% equ 0 set foo=1
goto:@F2 
:@F1
...
echo.%line_count%: %winws_arg_str_% >>%home_find%\run-!DOMAINS%coo%!-%_PORT%-%_HTTP%.%ext_old%
...
call:@create_strategy "%line_count%" "!DOMAINS%%a!"
:@F
set /a c_count+=1
if %c_count% leq %REPEATS% goto:@repeats_loop
goto:@F
:@F3
echo.[%date_runing%] %line_count%: %winwsdir%\winws.exe --debug=1 %wf_foo% --hostlist-domains=!DOMAINS%coo%! %winws_arg_str_% >>%home_find%\error-!DOMAINS%coo%!-%_PORT%-%_HTTP%.log
%winwsdir%\winws.exe --daemon %wf_foo% --hostlist-domains=!DOMAINS%coo%! %winws_arg_str_% 1>nul 2>"%temp%\run_cmd_find_strategy_winws_error"
...
echo.
echo.[%pos%G'winws.exe %winws_arg_str_%'
echo.
...
set "_dir=%_dir:[=%"
set "_dir=%_dir:]=%"
set "generate_to=%home_find%\%~2\%_HTTP%\%_dir%" 
if exist %generate_to% goto:@F
md %generate_to%\custom >nul 2>&1
md %generate_to%\skip >nul 2>&1
:@F
chcp 65001 >nul
if not exist %generate_to%\about echo.%lmess191% %_dir%: %~2:%_PORT% %_HTTP% >%generate_to%\about
...
REM # Тип 0x01 — ClientHello
set "clienthello_=01"
set "clienthello_len="
REM # Длина 3 байта
REM LegacyVersion (2) = 0x0303 - tls 1.2 (0x0304 - tls 1.3)
...
for /L %%a in (1,1,32) do (
	...
	for /f %%b in ("!rnd!") do set "Random_=!Random_!!chars:~%%b,1!"
    rem call set "Random_=!Random_!%%chars:~!rnd!,1%%"
)
REM echo.hex=[%hex%]
set /a chdec+=32
REM # Устарело
REM LegacySessionIdLength 1 байт
set "LegacySessionId=00"
set /a chdec+=1
REM # Устарело
REM # Длина списка наборов шифров — 2 байта
set "CipherSuitesLength=0006"
set /a chdec+=2
set "TLS_AES_128_GCM_SHA256=1301"
set /a chdec+=2
set "TLS_AES_256_GCM_SHA384=1302"
set /a chdec+=2
set "TLS_CHACHA20_POLY1305_SHA256=1303"
set /a chdec+=2
REM # Устарело
REM LegacyCompressionMethods 2 байта
set "LegacyCompressionMethods_=0100"
set /a chdec+=2
set "ExtensionsLength_=0009"
set /a chdec+=2
REM # Пример
REM 00 2b  # Тип расширения — supported_versions
REM 00 05  # Длина расширения 5 октетов
REM 04     # Длина списка версий 4 октета
REM 03 04  # Клиент поддерживает TLS 1.3
REM 03 02  # И TLS 1.1
set "Extensions_type=002B"
set /a chdec+=2
set "Extensions_type_len=0005"
set /a chdec+=2
set "Extensions_type_len_tls_ver=04"
set /a chdec+=1
set "Extensions_type_tls13=0304"
set /a chdec+=2
set "Extensions_type_tls11=0302"
set /a chdec+=2

set /a hdec=%chdec% + 4
set "handshake_len="
for /L %%a in (1,1,8) do (
	set /a "d=hdec&15, hdec>>=4"
	for /f %%b in ("!d!") do set "handshake_len=!chars:~%%b,1!!handshake_len!"
    REM rem call set "handshake_len=!handshake_len!%%chars:~!d!,1%%"
)
for /L %%a in (1,1,8) do (
	set /a "d=chdec&15, chdec>>=4"
	for /f %%b in ("!d!") do set "clienthello_len=!chars:~%%b,1!!clienthello_len!"
    REM rem call set "clienthello_len=!clienthello_len!%%chars:~!d!,1%%"
)
REM echo.handshake_len=[%handshake_len:~4%]
REM echo.clienthello_len=[%clienthello_len:~2%]
set "handshake_type=16"
set "old_msg=0303"
set "HandshakeMessage=0x%handshake_type%"
set "HandshakeMessage=%HandshakeMessage%%old_msg%"
set "HandshakeMessage=%HandshakeMessage%%handshake_len:~4%"
set "HandshakeMessage=%HandshakeMessage%%clienthello_%"
set "HandshakeMessage=%HandshakeMessage%%clienthello_len:~2%"
set "HandshakeMessage=%HandshakeMessage%%LegacyVersion_%"
set "HandshakeMessage=%HandshakeMessage%%Random_%"
set "HandshakeMessage=%HandshakeMessage%%LegacySessionId%"
set "HandshakeMessage=%HandshakeMessage%%CipherSuitesLength%"
set "HandshakeMessage=%HandshakeMessage%%TLS_AES_128_GCM_SHA256%"
set "HandshakeMessage=%HandshakeMessage%%TLS_AES_256_GCM_SHA384%"
set "HandshakeMessage=%HandshakeMessage%%TLS_CHACHA20_POLY1305_SHA256%"
set "HandshakeMessage=%HandshakeMessage%%LegacyCompressionMethods_%"
set "HandshakeMessage=%HandshakeMessage%%ExtensionsLength_%"
set "HandshakeMessage=%HandshakeMessage%%Extensions_type%"
set "HandshakeMessage=%HandshakeMessage%%Extensions_type_len%"
set "HandshakeMessage=%HandshakeMessage%%Extensions_type_len_tls_ver%"
set "HandshakeMessage=%HandshakeMessage%%Extensions_type_tls13%"
set "HandshakeMessage=%HandshakeMessage%%Extensions_type_tls11%"
REM set "HandshakeMessage=%HandshakeMessage%%%"

REM echo.HandshakeMessage=[%HandshakeMessage%]
REM pause
REM for /L %%a in (1,1,%len%) do (
	REM rem set /a rnd=(%RANDOM%*(max-min+1)/32768)+min
	REM set /a rnd=!RANDOM!*15 / 32768
	REM for /f %%b in ("!rnd!") do set "hex=!hex!!chars:~%%b,1!"
	REM rem for /L %%b in (!rnd!,1,!rnd!) do set "hex=!hex!!chars:~%%b,1!"
    REM rem call set "hex=!hex!%%chars:~!rnd!,1%%"
REM )
set %~1=%HandshakeMessage%
exit /b 0
