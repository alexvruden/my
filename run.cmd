...
if not exist %home%\script\run_agent.cmd (
	echo.
	echo.[5G[0mЗапуск из архива без распаковки невозможен. 
	echo.
...
set "win_ver="
set "win_ver7="
for /f "tokens=3* delims= " %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v ProductName ^| findstr "ProductName"') do set "os_name=%%a %%b"
if defined b_ma (
	set "win_ver_warn=[5G[31mРабота скрипта на этой системе не проверялась.[37m[E[5GПодробнее узнать тут:[E"
	set "win_ver_arch=[2m[5G[36mArchitecture[37m: %PROCESSOR_ARCHITECTURE% [36mOS[37m:"
	if %b_ma% lss 62 (
		set win_ver=!win_ver_arch! %os_name% ^(os updates may be required^)[E
		...
		set win_ver=!win_ver_arch! %os_name% ^(may require to disable secure boot^)[E
		set win_ver=!win_ver!!win_ver_warn!
		...
		if %b_mi% lss 22000 (
			...
			if "x%PROCESSOR_ARCHITECTURE%"=="xARM64" (
				set win_ver=!win_ver_arch! %os_name% ^(requires testsigning mode^)[E
				set win_ver=!win_ver!!win_ver_warn!
...
if %errorLevel% neq 0 ( 
	echo.
	echo.[5G[0mДля некоторых действий сценария необходимы высокие привилегии, запустите скрипт с правами 'Администратора'.
	echo.
	echo.[5GНажмите любую клавишу для выхода.
	pause >nul
	exit
...
if not "x%PROCESSOR_ARCHITECTURE%"=="xARM64" (
	if defined win_ver7 (
		...
		if "x%PROCESSOR_ARCHITECTURE%"=="xAMD64" (
			...
			if defined winwsdir (
				...
				if not exist %home%\bin\windows-7-amd64 (
					echo.
					echo.[5G[0mОфициальные бесплатные обновления Windows 7 [31mзакончились[0m в 2020. 
					echo.[5GПосле этого несколько лет продолжали идти платные обновления по программе ESU. 
					echo.[5GИменно в этих ESU обновлениях находится обновление ядра Windows 7, позволяющиее загрузить драйвер [33mwindivert 2.2.2-A[0m
					echo.[5GСкачать последнюю версию ESU: 22.2.10 https://nnmclub.to/forum/viewtopic.php?t=1530323
					echo.
					echo.[5GДля работы скрипта на [36mWindows 7 AMD64[0m, необходимо подготовить систему.
					echo.
					echo.[5GДля этого нужно: 
					echo.
					echo.[5G1. Создать директорию '[33m%homenc%\bin\windows-7-amd64[0m'
					echo.[5G2. Скопировать в нее файлы из директории '[33m%homenc%\!winwsdir:~%homestrsize%!\[0m'
					echo.[5G3. Заменить файлы '[33mWinDivert64.sys[0m' и '[33mWinDivert.dll[0m' на другие, для [36mWindows 7 AMD64[0m
					echo.
					echo.[5GПодробности тут:
					echo.[5Ghttps://github.com/bol-van/zapret/blob/master/docs/windows.md#windows-7-и-windivert
					echo.
					choice /N /C:nтyн /D n /T 300 /M "Позволить скрипту сделать эти действия? (y|n): "
					if !errorlevel! neq 255 (
						if !errorlevel! geq 3 (
							if exist %home%\bin\zapret-win-bundle-master\win7\WinDivert64.sys (
								echo.
								md %home%\bin\windows-7-amd64 1>nul 2>&1
								rem copying windows 7 compatible windivert 2.2.0-C
								copy /Y !winwsdir!\*.* %home%\bin\windows-7-amd64\*.* 1>nul 2>&1
								copy /Y %home%\bin\zapret-win-bundle-master\win7\WinDivert64.sys %home%\bin\windows-7-amd64 1>nul 2>&1
								copy /Y %home%\bin\zapret-win-bundle-master\win7\WinDivert.dll %home%\bin\windows-7-amd64 1>nul 2>&1
								copy /Y %home%\bin\zapret-win-bundle-master\win7\WinDivert64.sys %home%\bin\zapret-win-bundle-master\blockcheck\zapret\nfq 1>nul 2>&1
								copy /Y %home%\bin\zapret-win-bundle-master\win7\WinDivert.dll %home%\bin\zapret-win-bundle-master\blockcheck\zapret\nfq 1>nul 2>&1
								rem DONE.
							) else (
								echo.[5G[31mОшибка.[0m Отсутствует необходимый файл:
								echo.[5G'[33m%homenc%\bin\zapret-win-bundle-master\win7\WinDivert64.sys[0m'
								echo.[5G'[33m%homenc%\bin\zapret-win-bundle-master\win7\WinDivert.dll[0m'
								echo.[5GСкачайте архив с сайта разработчика и извлеките содержимое в папку '[33m%homenc%\bin\[0m'
								echo.[5Ghttps://github.com/bol-van/zapret-win-bundle
								echo.
								echo.[5GНажмите любую клавишу для продолжения.
								rem echo.[5GНажмите любую клавишу для выхода.
								pause >nul
								rem exit
							)
						)
					...
					set "winwsdir=%home%\bin\windows-7-amd64"
					set "arch=windows-7-amd64"
					set "archd=64"
					if not exist !winwsdir!\winws.exe (
						set "winwsdir="
					) else (
						for /f "delims=" %%I in ('!winwsdir!\winws.exe --version') do set "title_ver=%%I"
					)
				)
			)
			set "foo=1"
	...
	if defined winwsdir (
		set "fakedir=!winwsdir:~0,-24!\files\fake"
		if not exist %home%\bin\windows-arm64 (
			echo.[5G[0mДля [36mARM64[0m Windows нет подписанного драйвера '[33mwindivert[0m' и нет '[33mcygwin[0m'. 
			echo.[5GОднако, эмуляция [36mx64 Windows 11[0m позволяет использовать все, кроме '[33mWinDivert64.sys[0m' без изменений. 
			echo.[5GНо при этом надо заменить '[33mWinDivert64.sys[0m' на неподписанную [36mARM64[0m версию и установить режим '[36mtestsigning[0m'.
			echo.
			echo.[5GДля работы скрипта на [36mARM64[0m, необходимо подготовить систему.
			echo.
			echo.[5GДля этого нужно: 
			echo.
			echo.[5G1. Включить режим '[36mtestsigning[0m'
			echo.[5G2. Создать директорию '[33m%homenc%\bin\windows-arm64[0m'
			echo.[5G3. Скопировать в нее файлы из директории '[33m%homenc%\!winwsdir:~%homestrsize%!\[0m'
			echo.[5G4. Заменить файл '[33mWinDivert64.sys[0m' на другой, для [36mARM64[0m
			echo.[5G5. Перезагрузить систему, надпись "[36mтестовый режим[0m" должна быть на рабочем столе справа внизу.
			echo.
			echo.[5GПодробности тут:
			echo.[5Ghttps://github.com/bol-van/zapret-win-bundle#arm64-подготовка
			echo.
			choice /N /C:nтyн /D n /T 300 /M "Позволить скрипту сделать эти действия? (y|n): "
			if !errorlevel! neq 255 (
				if !errorlevel! geq 3 (
					if exist %home%\bin\zapret-win-bundle-master\arm64\WinDivert64.sys (
						echo.
						rem setting testsigning on
						set foo=0
						bcdedit /set TESTSIGNING ON 1>nul 2>&1
						if !errorlevel! equ 0 set foo=1
						rem replacing WinDivert64.sys with unsigned ARM64 version
						md %home%\bin\windows-arm64 1>nul 2>&1
						copy /Y !winwsdir!\*.* %home%\bin\windows-arm64\*.* 1>nul 2>&1
						copy /Y %home%\bin\zapret-win-bundle-master\arm64\WinDivert64.sys %home%\bin\windows-arm64 1>nul 2>&1
						copy /Y %home%\bin\zapret-win-bundle-master\arm64\WinDivert64.sys %home%\bin\zapret-win-bundle-master\blockcheck\zapret\nfq 1>nul 2>&1
						rem copying ip2net and mdig
						copy /Y %home%\bin\zapret-win-bundle-master\arm64\ip2net.exe %home%\bin\zapret-win-bundle-master\blockcheck\zapret\ip2net 1>nul 2>&1
						copy /Y %home%\bin\zapret-win-bundle-master\arm64\mdig.exe %home%\bin\zapret-win-bundle-master\blockcheck\zapret\mdig 1>nul 2>&1
						rem DONE. now reboot if testsigning was not enabled earlier.
						if !foo! equ 1 shutdown /r /t 60 /f 1>nul 2>&1
					) else (
						echo.[5G[31mОшибка.[0m Отсутствует необходимый файл '[33m%homenc%\bin\zapret-win-bundle-master\arm64\WinDivert64.sys[0m'
						echo.[5GСкачайте архив с сайта разработчика и извлеките содержимое в папку '[33m%homenc%\bin\[0m'
						echo.[5Ghttps://github.com/bol-van/zapret-win-bundle
						echo.
						echo.[5GНажмите любую клавишу для продолжения.
			...
			set "winwsdir=%home%\bin\windows-arm64"
...
rem bol-van: автоматическое включение tcp timestamps в windows, которые отключены по умолчанию
netsh interface tcp set global timestamps=enabled 1>nul 2>&1
echo.]0;[Bypassing Censorship] WinWS: %title_ver%\
cls
...
if %strategy_trigger% equ 0 (
	...
	for /f "delims=" %%I in ('2^>nul dir /b /a:d %home%\strategy\') do (
		...
		if !strategy_menu_count! neq 1000 (
			...
			if !fooc! geq 50 goto:@break_strategy_count
		)
	)
	if !foo! equ 0 ( 
...
:@break462
echo.[1F[2K#: %first_digit_char%%second_digit_char%
...
if not exist %home%\bin\zapret-win-bundle-master\cygwin\bin\bash.exe (
	echo.[%_pos%G[[31mx[0m][%pos%G[31mОшибка. [0mФайл не найден: '[33m%homenc%\bin\zapret-win-bundle-master\cygwin\bin\bash[37m'[0m
	goto:@err_blockcheck
)
if not exist %home%\bin\zapret-win-bundle-master\blockcheck\zapret\blockcheck.sh (
	echo.[%_pos%G[[31mx[0m][%pos%G[31mОшибка. [0mФайл не найден: '[33m%homenc%\bin\zapret-win-bundle-master\blockcheck\zapret\blockcheck.sh[37m'[0m
:@err_blockcheck
	echo.
	echo.[%pos%G[37mЕсли хотите использовать '[33mblockcheck[0m' то нужно его скачать и извлечь в директорию '[33m%homenc%\bin\[37m': [0m
	echo.
...
call:check_kill
echo.[%_pos%G[[33mi[0m][%pos%GОтредактируйте/Добавьте в '[33m%homenc%\blockcheck.config.txt[0m' параметры для сканирования.
echo.
echo.[%pos%GЕсли отредактировали, то нажмите любую клавишу для продолжения.
pause >nul
...
echo.
echo.[%_pos%G[[33mi[0m][%pos%GОтчет работы сохраняется в файл '[33m%homenc%\bin\zapret-win-bundle-master\blockcheck\blockcheck.log[0m'
(
echo.@echo off
echo.:loop
echo.cls
...
echo.[%_pos%G[[33mi[0m][%pos%GПосмотреть отчет можно с помощью '[33m%homenc%\blockcheck.log.cmd[0m'
echo.
echo.[%pos%GНажмите любую клавишу для возврата в меню.
pause >nul
goto:menu
		...
		if "x!fletter:~0,1!"=="x$" (
			...
			if "x!fletter!"=="xHOSTLIST" (
				if not exist %home%\lists\hostlist\hostlist-auto.txt (
					call:cecho xx3 "%str_file_path_for_cecho%\%%~I :" "Создан файл" "'%homenc%\lists\hostlist\hostlist-auto.txt'"
				...
				if "x%%~N"=="x" (
					...
					if not exist %home%\lists\hostlist\%%~N (
						for %%X in ("%%~N") do set foo=%%~nxX
						if not exist %home%\lists\hostlist\!foo! echo.#>%home%\lists\hostlist\!foo!
						set "profile_param=!profile_param! --hostlist-auto=%home%\lists\hostlist\!foo!"
					) else (
				...
				if "x%%~N"=="x" (
					...
					if exist %home%\lists\hostlist\%%~N (
						...
						for %%X in ("%%~N") do set foo=%%~nxX
						if not exist %home%\lists\hostlist\!foo! (
							call:cecho x1x3 "%str_file_path_for_cecho%\%%~I :" "Ошибка." "Файл не найден:" "'%homenc%\lists\hostlist\!foo!'"
							call:cecho xx31 "%str_file_path_for_cecho%\%%~I :" "Параметр" "!fletter!=%%~N" "отброшен"	
						) else (
							set "profile_param=!profile_param! --hostlist-auto=%home%\lists\hostlist\!foo!"
						)
				...
				if "x%%~N"=="x" (
					...
					if "x%IPsetStatus%"=="xon" (
						if exist %home%\lists\ipset\%%~N (
							...
							for %%X in ("%%~N") do set foo=%%~nxX
							if not exist %home%\lists\ipset\!foo! (
								call:cecho x1x3 "%str_file_path_for_cecho%\%%~I :" "Ошибка." "Файл не найден:" "'%homenc%\lists\ipset\!foo!'"
								call:cecho xx31 "%str_file_path_for_cecho%\%%~I :" "Параметр" "!fletter!=%%~N" "отброшен"	
							) else (
								set "profile_param=!profile_param! --ipset=%home%\lists\ipset\!foo!"
							)
						)
					) else (
						set "skip_profile=on"
			...
			if !parse_desync! equ 1 (
				...
				if "x!foo:~0,2!"=="x0x" (
					...
					if exist %fakedir%\%%~N ( 
						...
						for %%X in ("%%~N") do set foo=%%~nxX
						if not exist %fakedir%\!foo! (
							call:cecho x1x3 "%str_file_path_for_cecho%\%%~I :" "Ошибка." "Файл не найден:" "'%homenc%\!fakedir:~%homestrsize%!\!foo!'"
							call:cecho xx31 "%str_file_path_for_cecho%\%%~I :" "Параметр" "!fletter!=%%~N" "отброшен"	
						) else (
							set "profile_param=!profile_param! !fletter!=%fakedir%\!foo!"
						)
					)
				)
				set /a parse_mayok=1
...
for /F "skip=1 eol=# tokens=1,2 delims==" %%a in (%home%\blockcheck.config.txt) do (
	...
	if not "x!foo!"=="x" (
		set /a aoo=0
		for %%i in (CURL CURL_MAX_TIME ENABLE_HTTP TTPS_TLS12 ENABLE_HTTPS_TLS13 ENABLE_HTTP3 REPEATS HTTP_PORT HTTPS_PORT QUIC_PORT SCANLEVEL DOMAINS) do (
			if "x%%i"=="x!foo!" set /a aoo=1
		)
		if !aoo! equ 1 (
			set goo=%%b
...
if %CURL_MAX_TIMEw% lss 2 set "CURL_MAX_TIME=2"
...
if not "x!curl_version:~0,4!"=="xcurl" (
	...
	set foo=%home%
	set foo=!foo:\=/!
	echo.[%pos%GВ файле '[33m%homenc%\blockcheck.config.txt[0m' указать переменную '[36mCURL=!foo!/bin/curl.exe[0m'
	echo.
	echo.[%pos%G Скачать :
...
if defined CURL echo.[%pos%G[36mCURL=%CURLw%[0m
set "CURL=%CURLw%"
if defined SCANLEVEL echo.[%pos%G[36mSCANLEVEL=%SCANLEVEL%[0m
...
if "x%type_find%"=="x[HTTPS]" (
	...
	set curl_cmd_scan=-ISsk -A "%curl_user_agent%" --max-time %CURL_MAX_TIME% %TLSver% %TLSmax% https://%DOMAINS%
	set curl_cmd_scan_long=-Ssk -A "%curl_user_agent%" --max-time 60 %TLSver% %TLSmax% https://%DOMAINS%
...
if "x%type_find%"=="x[HTTP3]" (
	...
	set curl_cmd_scan=-Ss -A "%curl_user_agent%" --max-time %CURL_MAX_TIME% --http3-only https://%DOMAINS%
	set curl_cmd_scan_long=-Ss -A "%curl_user_agent%" --max-time 60 --http3-only https://%DOMAINS%
	set _PORT=%QUIC_PORT%
	set _HTTP=QUIC
)
if "x%type_find%"=="x[HTTP]" (
	...
	set curl_cmd_scan=-SsD %home%\bin\blk-hdr.txt -A "%curl_user_agent%" --max-time %CURL_MAX_TIME% http://%DOMAINS%
	set curl_cmd_scan_long=-SsD %home%\bin\blk-hdr.txt -A "%curl_user_agent%" --max-time 60 http://%DOMAINS%
	set _PORT=%HTTP_PORT%
	set _HTTP=HTTP
)
...
if %find_strategy_position_end% neq 0 (
	echo.[%_pos%G[[33mi[0m][%pos%GОбнаружен файл предыдущего поиска '[33m%homenc%\strategy\run-%DOMAINS%-%_PORT%-%_HTTP%.%find_strategy_position_end%[0m'
...
:@break1724
if exist %home%\strategy\error-%DOMAINS%-%_PORT%-%_HTTP%.log del /f /q %home%\strategy\error-%DOMAINS%-%_PORT%-%_HTTP%.log 1>nul 2>&1
...
<nul set /p =(B
echo.
set /a line_jump_num=0
echo.
set /a line_jump_num+=1
echo.[%_pos%G[[33mi[0m][%pos%GПри изменении прогресса поиска мы сохраним результаты в файл '[33m%homenc%\strategy\run-%DOMAINS%-%_PORT%-%_HTTP%.*[0m'
set /a line_jump_num+=1
...
for /F "skip=%line_begin% tokens=*" %%a in (%home%\strategy\strategy.lst) do (
	...
	set "jump=break"
	if "x!comment_char!"=="x[" goto:@find_strategy_
	set "foo="
	...
	set "jump=ping_error"
	%CURL% --max-time 4 one.one.one.one 1>nul 2>&1
	if !errorlevel! neq 0 goto:@find_strategy_
	...
	if !errorlevel! neq 0 (
		echo.!line_count!: %winwsdir%\winws.exe --dry-run !winws_arg_str_! >>%home%\strategy\error-%DOMAINS%-%_PORT%-%_HTTP%.log
	)
	call:check_run
	if !errorlevel! equ 0 (
		...
		for /l %%c in (1,1,%REPEATS%) do (
			...
			if %%c equ %REPEATS% (
				if !curl_ret_code! equ 0 (
					<nul set /p =8[%pos0%G!ap2!%%[%pos1%G!count_strategy![%pos2%G%total_strategy%[%pos3%G!find_strategy_found![%pos4%G!find_strategy_run_error![%pos5%G!find_strategy_kill_error![%pos6%G[%pos7%G[0mтест[?25l[8m
					%CURL% %curl_cmd_scan_long% 1>nul 2>&1
					if !errorlevel! equ 0 (
						set /a find_strategy_found+=1
						echo.!line_count!: !winws_arg_str_! >>%home%\strategy\run-%DOMAINS%-%_PORT%-%_HTTP%.!ext_old!
					...
					if !curl_ret_code_0! equ 0 if "x%SCANLEVEL%"=="xquick" (
						set /a find_strategy_found+=1
						echo.!line_count!: !winws_arg_str_! >>%home%\strategy\run-%DOMAINS%-%_PORT%-%_HTTP%.!ext_old!
						move /Y %home%\strategy\run-%DOMAINS%-%_PORT%-%_HTTP%.!ext_old! %home%\strategy\run-%DOMAINS%-%_PORT%-%_HTTP%.!count_strategy! 1>nul 2>&1
						set ext_old=!count_strategy!
	...
	if !errorlevel! neq 3 goto:@find_strategy_
)
set "jump=break"
goto:@find_strategy_
...
if %find_strategy_found% neq 0 (
	...
	echo.[%pos%GОписание стратегий в файле '[33m%homenc%\strategy\done-%DOMAINS%-%_PORT%-%_HTTP%.log[0m'
)
goto:@find_strategy_end
...
echo.[%pos%G[31mОшибка запуска WinWS.[0m
goto:@find_strategy_end_error
:@find_strategy_kill_error
echo.[%pos%G[31mОшибка завершения WinWS.[0m
goto:@find_strategy_end_error
:@find_strategy_ping_error
echo.[%pos%G[31mСбой проверки интернета.[0m
goto:@find_strategy_end_error
:@find_strategy_end_error
echo.[%pos%GПоиск ставим на паузу. Попробуйте продолжить поиск.
:@find_strategy_end
echo.[%pos%GНажмите любую клавишу для возврата в меню.
echo.[?25h
pause >nul
...
if not exist %generate_to% ( 
	md %generate_to%\custom >nul
	md %generate_to%\skip >nul
)
if not exist %generate_to%\about echo.Создана скриптом из списка [line: %~1] %DOMAINS%:%_PORT% %_HTTP% >%generate_to%\about
echo.#Create from list line: %~1 >%generate_to%\%_PORT%-%_HTTP%.strategy
for /l %%i in (%generate_count%,-1,0) do (
	if defined generate_str%%i echo.!generate_str%%i!>>%generate_to%\%_PORT%-%_HTTP%.strategy
)
echo.--hostlist-auto=hostlist-%DOMAINS%.txt>>%generate_to%\%_PORT%-%_HTTP%.strategy
...
if not exist %home%\lists\hostlist\hostlist-%DOMAINS%.txt echo.%DOMAINS%>>%home%\lists\hostlist\hostlist-%DOMAINS%.txt
if not exist %generate_to%\80-HTTP.strategy (
	...
	echo.--hostlist-auto=hostlist-%DOMAINS%.txt
...
if not exist %generate_to%\443-HTTPS.strategy (
	...
	echo.--wf-tcp=443
	echo.--hostlist=hostlist-%DOMAINS%.txt
...
if exist %home%\blockcheck.config.txt exit /b
echo.[%_pos%G[[31mx[0m][%pos%GНет файла параметров. Создадим его.
chcp 65001 >nul
...
if %errorLevel% equ 0 (
	...
	set foo=%home%
	set foo=!foo:\=/!
	if exist %home%\bin\curl.exe set CURLb=CURL=!foo!/bin/curl.exe
	if exist %home%\curl.exe set CURLb=CURL=!foo!/curl.exe
)
(
echo.## 
echo.# РџСѓС‚СЊ Рє curl.exe РїРёС€РµС‚СЃСЏ РІ unix С„РѕСЂРјР°С‚Рµ, РѕР±СЂР°С‚РЅС‹Р№ СЃР»СЌС€
echo.# РџСЂРёРјРµСЂ: C:/folder/who/curl.exe
echo.# РџР°РїРєРё СЃ РЅР°С†РёРѕРЅР°Р»СЊРЅС‹РјРё СЃРёРјРІРѕР»Р°РјРё Рё РїСЂРѕР±РµР»Р°РјРё РЅРµ РїСЂРёРјРµРЅСЏС‚СЊ
echo.# РЎРєСЂРёРїС‚ РїСЂРѕРІРµСЂРёС‚ РЅР°Р»РёС‡РёРµ РІСЃС‚СЂРѕРµРЅРЅРѕРіРѕ РІ Windows curl, РµСЃР»Рё РµРіРѕ РЅРµС‚, 
echo.# С‚Рѕ Р±СѓРґРµС‚ РёСЃРїРѕР»СЊР·РѕРІР°С‚СЊ РёР· РґРёСЂРµРєС‚РѕСЂРёРё bin/curl.exe Рё СЃР°Рј РїСЂРѕРїРёС€РµС‚ РїСѓС‚Рё РІ СЌС‚РѕРј РєРѕРЅС„РёРіРµ.
echo.# РљРѕРЅС„РёРі СЃРѕР·РґР°РµС‚СЃСЏ РѕРґРёРЅ СЂР°Р·, РµСЃР»Рё С‡С‚Рѕ-С‚Рѕ РёСЃРїРѕСЂС‚РёР»Рё Р·РґРµСЃСЊ, СѓРґР°Р»РёС‚Рµ СЌС‚РѕС‚ С„Р°Р№Р», СЃРєСЂРёРїС‚ СЃРѕР·РґР°СЃС‚ РЅРѕРІС‹Р№.
echo.
...
echo.
echo.DOMAINS="ntc.party discord.com rutracker.net x.com"
)>%home%\blockcheck.config.txt
chcp 1251 >nul
echo.[%_pos%G[[32m+[0m][%pos%GСоздан файл параметров '[33m%homenc%\blockcheck.config.txt[0m'
exit /b