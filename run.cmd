...
set "foo=[32m"
set /a meta=0, cb=0
set /a c1=5, c2=9, c3=13, c4=30, c5=55, c6=80, c7=100
:@loopL9
set /a sf=0, fb=0
...
:@F
if exist %home%\lang\%OSLanguage% goto:@F
echo.[5G[31mNot found language file in '%home%\lang\'[m
echo.
echo.[5G%hoo%
pause >nul
exit /b 1
...
for /F "skip=1 eol=# tokens=1,2 delims==" %%a in (%home%\run.config) do (
	set foo=%%a
	set /a aoo=0
	for %%i in (set_secure_dns term_trigger srv_trigger strategy_trigger param_trigger show_popup language new_version_available check_update eula daemon custom_strategy IPsetStatus ip_router ip_test agent_mode agent_start_strategy agent_start_params) do (
...
:@F
set /a goo=0
set /a labels_total=0
for /F "usebackq delims=" %%a in (`"findstr ^^ %home%\run.cmd"`) do (
	set "foo=%%a"
	if !goo! equ 0 (
	...
	set foo=!foo: =!
	if "x!foo:~0,1!"=="x:" (
		set /a labels_total+=1
		set "labels_array!labels_total!=!foo:~1!"
	)
)
...
echo.
echo.[32m[i] [95mrun.cmd [32m%arg_1%[m "[36m%arg_2%[m" %arg_3%
echo.
goto:@terminate
:@F
if not "x%arg_1%"=="xstop" goto:@F
echo.
echo.[32m[i] [95mrun.cmd [31m%arg_1%[m
echo.
goto:@terminate
:@F
...
:@F
if not "x%set_secure_dns%"=="xon" netsh %_dnsclient% set global doh=no 1>nul 2>&1
cd /d %home%
...
:@F
2>nul %_pwsh% -Command "Get-WmiObject win32_process -Filter 'name = \"cmd.exe\"' | select commandline" |find "run.cmd" |find "agent" 1>nul 2>&1 &&goto:@F
if exist %home%\agent.log del /F /Q %home%\agent.log 1>nul
:@F
...
for /f "tokens=1* delims=:" %%a in ('2^>nul %_curl% --max-time 10 -sH "Accept: application/vnd.github+json" https://api.github.com/repos/alexvruden/my/releases/latest ^|findstr "updated_at"') do (set "new_updated=%%b")
chcp 65001 >nul
for /f "tokens=1* delims=:" %%a in ('2^>nul %_curl% --max-time 10 -sH "Accept: application/vnd.github+json" https://api.github.com/repos/alexvruden/my/releases/latest ^|findstr "body"') do (set "new_updated_changelog=%%b")
chcp %lmess1% >nul
for /f "tokens=1* delims=:" %%a in ('2^>nul %_curl% --max-time 10 -sH "Accept: application/vnd.github+json" https://api.github.com/repos/bol-van/zapret/releases/latest ^|findstr "tag_name"') do (set "new_updated_winws=%%b")
if not defined new_updated_winws goto:@F
	...
	for /F "usebackq tokens=1* delims=:" %%a in (`"findstr /n ^^^^ !foo!"`) do (
		...
		if "x!fletter:~0,1!"=="x$" (
			...
			if "x!fletter!"=="xHOSTLIST" (
				...
				if "x!LN:~0,1!"=="x@" (
				...
				) else if "x!LN:~0,4!"=="xgen:" (
					rem generate raw filter
					set "WinDivert_generate_filter=!LN:~4!"
					set "psabout=!LN:~4!"
					...
					REM set "sWinDivert=--wf-raw=!sletter!"
					set "sWinDivert=--wf-raw: error not support string filter"
					set "skip_WinDivert=on"
					set /a pause_on_error=1
					call:@cecho -xx31 "%%~I [!parse_str_lcount!]:" "%lmess124%" "!fletter!=!sletter!" "%lmess125%"
					REM call:@cecho -x1x "%%~I [!parse_str_lcount!]:" "%lmess126%" "WinDivert %lmess127% --wf-raw"
					call:@cecho -x1x "%%~I [!parse_str_lcount!]:" "%lmess126%" "--wf-raw: error not support string filter"
				)
				if "x!sabout!"=="x" (
				...
				set "LN="
				set "psabout="
			) else if "x!fletter!"=="x--wf-raw-part" (
				set "LN=!sletter!"
				if "x!LN:~0,1!"=="x@" (
					if exist %parse_str_strategy_apath%\!LN:~1! (
						set sWinDivert=--wf-raw-part=@%parse_str_strategy_apath%\!LN:~1!
					) else (
						set "skip_WinDivert=on"
						set /a pause_on_error=1
						call:@cecho -x1x3 "%%~I [!parse_str_lcount!]:" "%lmess20%." "%lmess21%:" "'%homenc%\strategy\%strategy_name%\!LN:~1!'"
					...
					REM set sWinDivert=--wf-raw-part=!sletter!
					set "sWinDivert=--wf-raw-part: error not support string filter"
					set "skip_WinDivert=on"
					set /a pause_on_error=1
					call:@cecho -xx31 "%%~I [!parse_str_lcount!]:" "%lmess124%" "!fletter!=!sletter!" "%lmess125%"
					REM call:@cecho -x1x "%%~I [!parse_str_lcount!]:" "%lmess126%" "WinDivert %lmess127% --wf-raw-part"
					call:@cecho -x1x "%%~I [!parse_str_lcount!]:" "%lmess126%" "--wf-raw-part: error not support string filter"
				)
				if "x!sabout!"=="x" (
					if "x!psabout!"=="x" ( set "sabout=!sWinDivert:~5!" ) else ( set "sabout=!psabout!" )
				) else (
			...
			if !parse_desync! equ 1 (
				...
				if "x!foo:~0,2!"=="x0x" (
					...
					call:@rnd_tls_gen foo coo 
					if !errorlevel! equ 0 (
						call:@cecho -x2 "%%~I [!parse_str_lcount!]:" "!rnd_tls_gen_info!"
					...
					if exist %fakedir%\!sletter! ( 
						...
						if not exist %fakedir%\!hoo! (
							set "profile_param=!profile_param! !fletter!=!sletter!"
							REM call:@cecho -x1x3 "%%~I [!parse_str_lcount!]:" "%lmess20%." "%lmess21%:" "'..\!fakedir:~%homestrsize%!\!hoo!'"
							REM call:@cecho -xx31 "%%~I [!parse_str_lcount!]:" "%lmess124%" "!fletter!=!sletter!" "%lmess125%"	
						) else (
...
:@check_labels
set "check_labels_foo=%~1"
for /L %%a in (1,1,%labels_total%) do if "x!labels_array%%a!"=="x%check_labels_foo%" exit /b 0
exit /b 1
...
exit /b
:@F3
set "create_strategy_foo=%~2"
set "create_strategy_foo=%create_strategy_foo:.=-%"
if not exist %generate_to%\about.%create_strategy_foo% echo.%lmess191% %list_line%: %~2:%_PORT% %_HTTP% %~3 >%generate_to%\about.%create_strategy_foo%
exit /b
...
call:@check_labels "@rnd_tls_gen_%rnd_tls_gen_hoo%" ||(call:@cerror check_labels_rnd_tls_gen_error &exit /b 1)
...
exit /b 0
:@rnd_tls_gen_tls11
set "LegacyVersion_=0302"
set "rnd_tls_gen_tlsv_str=TLS 1.1"
goto:@F
:@rnd_tls_gen_tls13
set "LegacyVersion_=0304"
set "rnd_tls_gen_tlsv_str=TLS 1.3"
goto:@F
:@rnd_tls_gen_tls12
set "rnd_tls_gen_tlsv_str=TLS 1.2"
...
if defined Extensions_type_server_name_ascii_string set "rnd_tls_gen_info=%rnd_tls_gen_tlsv_str% sni=%Extensions_type_server_name_ascii_string%"
...
if not defined lmess1001_one_time_show call:@message log "%lmess1001%"
rem set "lmess1001_one_time_show=done"
:@F
set "host_e=1.1.1.1"
if not defined ip_test goto:@F
set "host_e=%ip_test%"
:@F
if not defined lmess1001_one_time_show call:@message log "test external ip - %host_e%"
set "lmess1001_one_time_show=done"
...
set "strategy_run="
if %stop_no_inet% equ 0 set "is_agent_stop_no_ping=run"
